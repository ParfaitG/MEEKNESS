...
class GUIapp():
    ...
    def dbConnect(self, orig):
        ...
        # DB CONNECT
        self.db = pymssql.connect(server='*****', user='***', password='***', database='Meekness', port=1433)         
        ...
        sql = """SELECT c.[Character], c.[Picture], c.Quote
                 FROM [Characters] c
                 WHERE c.[Character] = %s;"""
	...
	# DATABASE IMAGE
        self.dbimglbl = Label(self.guiframe, text='                SQL Server', font=("Arial", 14)).\
                         grid(row=0, column=0, sticky=W, padx=5, pady=5)

        self.dbimg_obj = Image.open('SQLServer.jpg')
