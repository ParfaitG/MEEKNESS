...
mainWindow <- function(times=1, charnum='Beetle Bailey'){
  ...
  tbl[1,1] <- gimage(filename = "SQLite.gif", dirname = getwd(), container = tbl)
  
  tbl[2,1] <- glabel("SQLite", container = tbl)
  font(tbl[2,1]) <- list(size=14, family="Arial")
  
  # CHARACTER DROP DOWN
  charlist <- getList()
  
  tbl[3,1] <- charcbo <- gcombobox(charlist[[2]], selected = 1, editable = TRUE, 
